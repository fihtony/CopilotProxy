...
test.describe("Stats tracking", () => {
  ...
  async function sendRequest(request: Parameters<typeof test.beforeAll>[0], content = "test") {
    return request.post(`${CLIENT_URL}/api/v1/chat/completions`, {
  ...
  test("TC-STATS-01: successful requests increment total calls", async ({ request }) => {
    // Baseline
    const before = await request.get(`${ADMIN_URL}/api/admin/keys/${keyId}/stats?window=24h`);
    ...
    const after = await request.get(`${ADMIN_URL}/api/admin/keys/${keyId}/stats?window=24h`);
  ...
  test("TC-STATS-03: response time is recorded", async ({ request }) => {
    ...
    const statsResponse = await request.get(`${ADMIN_URL}/api/admin/keys/${keyId}/stats?window=24h`);
  ...
  test("TC-STATS-04: token counts are stored in history", async ({ request }) => {
    ...
    const histResponse = await request.get(`${ADMIN_URL}/api/admin/keys/${keyId}/history?page=1&limit=1`);
    const { items } = await histResponse.json();
    expect(items.length).toBeGreaterThan(0);

    const row = items[0];
    // Echo mode may return 0 token counts; verify values are non-negative numbers and sum correctly
    ...
    expect(row.completion_tokens).toBeGreaterThanOrEqual(0);
  ...
  test("TC-STATS-05: overview aggregates across keys", async ({ request }) => {
    ...
    const overview = await request.get(`${ADMIN_URL}/api/admin/overview?window=24h`);
    ...
    await request.delete(`${ADMIN_URL}/api/admin/keys/${item2.id}`);
  });

  test("TC-STATS-06: time window 30d totals >= 24h totals", async ({ request }) => {
    const r24h = await request.get(`${ADMIN_URL}/api/admin/overview?window=24h`);
    const r30d = await request.get(`${ADMIN_URL}/api/admin/overview?window=30d`);
