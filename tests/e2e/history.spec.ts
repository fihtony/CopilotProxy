...
test("TC-HIST-01/02: shows request history rows and forced model usage", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "/api/v1/chat/completions" }).first()).toBeVisible();
...
test("TC-HIST-03: pagination returns correct rows", async ({ request }) => {
  ...
  const page1 = await request.get(`${ADMIN_URL}/api/admin/keys/${item.id}/history?page=1&limit=20`);
  const page2 = await request.get(`${ADMIN_URL}/api/admin/keys/${item.id}/history?page=2&limit=20`);
